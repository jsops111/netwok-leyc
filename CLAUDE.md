...
**但"跑在别的拍子上"不等于只能用瞬时事件。**`process()` 现在收一个
`scope`(一组 kind):给了它之后**只有落在 scope 里的事件参与开关**,别的
一律不碰。于是配置合规(跑在备份那一拍)、规则审计(跑在策略同步那一拍)
也能拿到"自动开 + 自动关"这套语义 —— 而那正是它们需要的:
**"telnet 一直开着"有持续状态**,修好之前一直是个问题,修好之后应该自动恢复。
瞬时事件给不了这个。

判断该用哪个:**这个问题会不会"好起来"?**会 → `process(scope=...)`;
不会(配置变更、备份失败、邻居变化,发生了就是发生了)→ `record_point_event()`。

传进 `process(scope=...)` 的 problems 里混进作用域外的 kind 会被**忽略并记
一条 WARNING** —— 放行的话它会开出一条**永远关不掉**的事件(下一拍它不在
scope 里,不会被当成已恢复)。
...
devices/sdwan.py  **纯函数**:SD-WAN 健康检查的响应 → 每条链路一行。
...
idrac/redfish.py  只发 HTTP,不解析业务语义。每个端点独立 try,只有 system 致命
idrac/parse.py    **纯函数**:Redfish JSON → 部件模型。unknown ≠ ok 那条在这
idrac/collector   连接、判阈值、写库、开事件。evaluate_idrac() 是唯一的
                  **带外**阈值判定处;它的 add() 是**合并**语义(同 kind 只能一条)
```
...
`IdracHost` → `IdracSample`。走 **Redfish**(HTTPS REST,只读),不走 SSH、
不走 SNMP。

### 和「服务器」是两件事,不是一张表的两列

`Server` 走 SSH,答"这台机器上跑的系统忙不忙、盘满没满"—— 它**看不见物理
部件**:一块正在 SMART 预警的硬盘、一条报了可纠正错误的内存、一个已经掉了电
的冗余电源,在操作系统里通常**一点症状都没有**。

`IdracHost` 走带外,答"这台机器本身会不会坏"。它反过来不知道机器上跑的是什么,
甚至不知道开没开机。

两边覆盖不重合,所以谁都不能做成谁的附属列:一台裸金属可能只有 iDRAC 没有
SSH 账号,一台云主机只有 SSH 没有 iDRAC。`IdracHost.server` 那个外键是**可选**的,
填了页面上两张卡片连起来,不填照样是一台完整的被监控对象。

`Event` 那边同理:一台机器可以同时开着 `disk_high`(带内,盘满了)和
`hw_disk`(带外,一块物理盘要坏了)—— 两条都对,而且要分别开关。所以
**加了第五个来源外键 `Event.idrac`**,那条部分唯一索引的 `fields` 里也补了
一行(见第 9 条:漏补的表现很安静 —— 同一块坏盘每一拍开一条新事件)。
加列本身不会制造冲突(新列在老行上全是 NULL,老行之间原本就互相区分开),
所以 migration 里**不需要**先去重;加 `server` 那次要去重是因为在那之前
那条约束根本没生效过。
...
`temp_delta_warn_c`(同机温差)是平台自己算的判据 —— 而且它能把"机房热"和
"这一颗坏了"分开:进风温度(`Inlet Temp`)不高而两颗差很多 = 是这一颗。

同理 `FailurePredicted`(SMART 预警)在 iDRAC 的整机健康里通常只是个 warning,
但"这块盘还在跑、但快坏了"恰恰是最该立刻换的一条 —— 等 `Health` 变红时盘
通常已经掉了。

⚠ **`\bcpu\b` 这个正则匹配不到 `CPU1 Temp`。**"CPU" 和 "1" 之间没有单词
边界(两边都是 `\w`)。实测踩出来的:加同机温差判据时它静默失效了,而
"算不出温差"和"两颗一样热"在页面上长得一样。用 `cpu|proc`。

### 三个"不能混"的三态

| 情况 | 值 | 为什么 |
|---|---|---|
| 部件读不到状态 | `unknown`,**单独一栏** | 「读不到」和「是好的」是两个结论。并进 ok 会让一台权限配错的 iDRAC 在大屏上显示成一片绿 —— 而那正是最需要有人去看一眼的时候 |
| 机械盘的剩余写入寿命 | `None` | HDD 没有这个概念,Redfish 上它返回 0。不 join `MediaType` 就下结论的话,一台插满机械盘的机器会报一排"SSD 寿命耗尽" |
| 刚加进来还没轮到采 | `pending`,**不算故障** | 算成"失联"会让新加的机器先红一阵子,人会去查一个不存在的问题 |

大屏上「带外失联」和「硬件有严重告警」也是**两栏,不合并**:前者去查网络/
凭据,后者去机房换件 —— 合成一栏会让人先跑错方向。

### SEL(硬件事件日志)不会自动清

一台跑了几年的机器上留着很早以前的记录很正常。直接数"多少条 critical"会得到
一个永远不变的大数字 —— **一条永远都在的红等于没有红**,人看两次就不看了,
真出事的那条就淹在里面。所以按 `event_window_days` 过滤,而时间戳解不出来的
条目**保留但不计入计数**(我们不知道它是什么时候的,不能替它决定算不算数)。

### 同一个 kind 只能有一条 problem

`event_engine.process()` 里是 `{p["kind"]: p for p in problems}` —— **同 kind
的第二条会静默覆盖第一条**。而带外这边一个 kind 下天生就有好几种发现
(温度既有绝对值超线又有同机温差、盘既有已经坏的又有 SMART 预警)。
按追加写的话页面上只会看到最后一条,**而最后一条恰恰常常是级别较低的那条** ——
实测:89℃ 的 critical 被 20℃ 温差的 warning 顶掉了。

所以 `evaluate_idrac()` 里的 `add()` 是**合并**语义:级别取高的,消息拼起来,
高级别那句放前面。加新判据时不要绕开它。

### 打 BMC 要克制

采集间隔**最小 60 秒**(模型 validator 挡着),beat 30 秒敲一次派发。
BMC 是一颗很弱的处理器,打太勤会把它自己拖慢,严重时管理界面登不进去 ——
**而那正是出事时要用的东西**。一次采集复用同一个 `requests.Session`
(TLS 握手是最贵的一步,五个端点各握一次手会让一次采集从 1 秒变 3 秒)。

`esxcli` 那边的教训在这里也适用:**每个端点独立 try**,只有 `system` 段是
致命的。iDRAC 7/8 上 `$expand` 不支持是常态 —— 那时逐条内存拿不到,
只剩汇总健康值,页面上要说明白为什么没有明细,**而不是显示"内存 0 条"**。

`verify_tls` 默认**关**:iDRAC 出厂是自签证书。关掉时显式 disable 掉
urllib3 的 InsecureRequestWarning —— 几十台机器每分钟一条会把日志淹掉。
...
`Device.collect_sdwan` → `SdwanLink` / `SdwanSample`。FortiGate 专有。

### ⚠ 它和线路拨测测的不是同一段

`ProbeTarget` 的 latency / loss / jitter 是**这个平台自己**从部署点探出来的;
`SdwanLink` 的三个数是**防火墙自己**从它的出口探出来的(FortiOS 的
health-check,默认 500ms 一拍)。

同一条运营商线路,**两边测出来的数不一样是正常的** —— 路径不同。而两边都有
才分得清:

    防火墙侧正常、平台侧不通  →  我们到防火墙这一段的问题
    两边都不通               →  那条线路真的断了

**这一页不是拨测的替代,是另一个视角。**这句话写在页面顶部 —— 不写的话
两个数对不上时人会以为其中一个坏了,然后去查一个不存在的故障。

### 跟着设备采集的节拍,不单独排一类调度

它是**指标**(延迟/抖动/丢包),和 CPU、温度同一个性质 —— 每台设备自己的
`interval_seconds` 已经是对的节拍,再加一类 zset 只是多一处要维护的东西
(对比:备份和策略同步要单独排,因为它们的周期是小时/分钟级,和采集不是
一个量级)。

它的 problems **合进设备那一次 `process()`**,不另开一次 —— `process()` 的
语义是"这个来源这一拍观测到的全部问题",分两次调用的话后一次会把前一次
开的事件当成已恢复关掉(第 8 条)。

### 达标判定以设备为准,平台不重算

FortiOS 允许一个健康检查配**多档 SLA**(sla 1 要求 100ms、sla 2 要求 200ms),
选路按档走 —— **它比我们更清楚它按哪一档选路**。所以 `sla_met` 直接用设备
报的,平台不拿门限重算一遍。

平台自己那两个门限(`Device.sla_latency_warn_ms` / `sla_loss_warn_pct`)是
**额外**的一层,给"设备说达标但数字已经很难看"准备的:FortiOS 的门限常常
配得很松(比如 200ms),而一条 180ms 的专线该早点有人看一眼。
**告警文案里要写明那是平台的线不是设备的** —— 否则人会去 FortiGate 上找
一条对不上的 SLA 配置。而且它**只在设备说达标时才判**,不然同一条链路
会报两遍。

### 同一个 kind 只能有一条 problem

和带外那边**同一个坑**:`event_engine.process()` 里是
`{p["kind"]: p for p in problems}` —— 同 kind 的第二条会**静默覆盖**第一条。

SD-WAN 这边一个 kind 下天生就有好几条:一台设备上 wan2 和 wan3 可以同时
SLA 未达标、两个成员可以同时断。按追加写的话事件里只会看到最后一条,
**而最后一条常常是级别较低的那条** —— 实测:wan2 的"2 档 SLA 都没过"
(真正的 SLA 违规)被 wan3 的"设备说达标但超平台软门限"顶掉了。

所以 `_collect_sdwan()` 里那个 `add()` 是**合并**语义,和
`idrac/collector.evaluate_idrac()` 里那个是同一份:级别取高的,消息拼起来,
高级别那句放前面。**加新判据时不要绕开它。**

(这个坑在这个代码库里已经出现两次了 —— 凡是"一个 kind 下可能有多个发现"
的采集器都要走合并,别再写第三份 `problems.append`。)

### 三态和「不能填 0」

| 情况 | 值 | 为什么 |
|---|---|---|
| 状态认不出来 | `unknown` | 「这一拍没读到这个成员」和「它是通的」是两个结论 |
| 设备没报 SLA 判定 | `sla_met = None` | 没配 SLA 目标 / 固件不给这一项。显示成"达标"就是替设备做一个它没做的判断 |
| **不通的那一拍** | 延迟/抖动 `None`,丢包 `100` | 和拨测同一条规矩。**设备在 dead 时往往还回着上一次的延迟值**,原样存下来会让一条已经断了的线在图上显示成"延迟很正常" |
| SSH 通道的 SLA 档数 | `None` | 那条通道只有 `sla_map` 位图,看得出过了哪几档但看不出一共配了几档。填 0 会被读成"配了 0 档" |

「只看有问题的」筛的是**不通 + 明确未达标**,`sla_met = None` 的**不算** ——
算进去的话一台没配 SLA 目标的防火墙会整页飘红。

### 一行是「健康检查 × 成员」

FortiOS 里一个健康检查会同时探**所有成员**(wan1 / wan2 / ipsec…),而 SLA 是
**按成员判**的:wan1 达标、wan2 不达标是最常见的情形,也正是 SD-WAN 要做选路
的原因。合成"一条线路一行"就看不出是哪个口掉了。

顺带:`ISP1/wan1` 和 `ISP2/wan1` 是两条不同的链路 —— 拿 `member` 当唯一键
会让它们互相覆盖(写测试时踩到过)。

### 通道

- **API(推荐)**:`/monitor/virtual-wan/health-check` 一次给全部成员的
  延迟/抖动/丢包/达标/带宽/会话数。**端点名在大版本间变过**,按新到老
  依次试 —— 只试一条的话老固件上这一页永远是空的,**而且不报错**
- **SSH 兜底**:`diagnose sys sdwan health-check`(7.0 之前叫
  `diagnose sys virtual-wan-link health-check`,画像里两条都写、按顺序试)。
  **拿不到的比拿得到的多**:没有带宽、没有会话数、没有 SLA 档数总数

响应形状认三种(`{检查: {成员: {}}}` / `members` 数组 / 已拍平)——
和 `_extract_usage()` 同一条规矩:只认一种的话升级固件后这一页会静默变空。
...
监控类、线路、设备卡片、服务器卡片**全都按 `order` 升序**(数字小的在前,
0 是最高优先级),和配置中心、下拉、事件页一致 —— 同一台设备在四个页面上
排四种顺序,人对不上。

线路那里有个坑:一张图最多画 `max_lines` 条,**纯按优先级截的话,一条优先级
排在第 20 位、正在断的线会被截掉**,大屏上完全看不见 —— 那是这一屏最不该
发生的事。所以是**先挑后排**:挑的时候有问题的优先进来,进来之后一律按
优先级排。两件事分开做,各自都对。

⚠ **设备卡片那里还有一个坑,而且后端排对了也没用。**`/dashboard/devices/`
是按 kind 分成三个桶给的(`switches` / `firewalls` / `others`),**每个桶内**
按优先级排好 —— 但前端 `[...switches, ...firewalls, ...others]` 拼完之后
**整体不是优先级顺序**:一台优先级 5 的交换机会排在优先级 1 的防火墙前面,
因为它在前一个桶里。而这一屏是一个网格、不按类型分区,所以拼完必须重排。

这类"后端排对了、前端拼一下就废了"的问题在接口按类型分桶时很容易发生 ——
**改排序时要顺着数据一路看到渲染那一行**,只改 `order_by` 是不够的。
...
大屏**一次刷新只打四个聚合接口**(`overview` / `charts` / `devices` /
`sdwan/board?hours=0`)。**别改成每条线路一个请求** —— 几十条线路 ×
每 5 秒会把 gunicorn 打满。这条规矩管的是"不要按实体扇出",不是"接口数
不能超过三个":SD-WAN 那个也是一次聚合、而且 60 秒一拍(带外和 SD-WAN
变化都慢),在这条规矩的射程之内。

`hours=0` 那个参数是有意的:board 接口默认会给每条链路带一小时的采样点,
而大屏那几张小卡**只显示当前值、不画趋势** —— 50 条链路 × 60 个点每分钟
白传一遍。趋势在 `/sdwan` 那一页看。

### 大屏上"显示哪些"存浏览器,不存库

设备和 SD-WAN 链路都能在大屏上勾选,选择**存 localStorage**:一块挂在墙上
的大屏和一个人在工位上开的页面**要看的东西不一样**,而存库是一份**共享**
设置 —— 两个人各调各的会互相覆盖,大屏上的东西莫名其妙变掉还查不出是谁改的。

(对比:保留期是存库的 —— 那是**系统**设置,全站只该有一份;
"这块屏显示什么"是**视图**设置,天生该按屏各算各的。)

存的是**"隐藏哪些"而不是"显示哪些"**:新加的设备默认就该出现在大屏上,
存白名单的话它会悄悄不显示。

⚠ **被隐藏的里面有正出问题的,必须点名。**这是这个功能唯一危险的地方:
有人把一台设备从大屏上藏掉、然后它坏了,而大屏上一点痕迹都没有。所以
"隐藏了 N 台"一直写着,**其中有问题的那几台点名**。事件页和告警推送本来
就不受这个开关影响,但大屏是最常被盯的那一块,不能让它骗人。

选择框里还要写死一句「**隐藏 ≠ 停止监控**」—— 不写的话一定会有人用隐藏
来"关掉"一台设备。
...
### 堆叠(StackWise)要画成**两块机箱**

一对堆叠的 C9300 上口名是 `Gi1/0/1..48` + `Gi2/0/1..48`。`PortBank.pattern`
里**必须有两个捕获组 `(成员号, 口号)`** —— 只抓口号的话两台的第 30 口都抓出
30,于是 96 个口挤成一排、因为重号被重新编号,而**人照着数第 30 个格子会落在
另一台交换机上**。照着它去拔线,拔的是别人的。

所以 `build()` 按成员号分块,前端**一台机箱一个 SVG**(上下两块,不横着接
成一排),铭牌上印 `SW 1` / `SW 2`。**单台不加「成员 N」前缀** —— 那是堆叠
才有的概念,单台标着它人会去想"那成员 2 呢"。

### 实体口 / 逻辑口要分开数

一台 48 口交换机的接口表里往往还有几十个 Vlan 和一堆 Port-channel。混在一个
「接口数」里的话,**页面上的数字和人在机柜前面能数出来的口数对不上** ——
而"对不上"这件事本身会让人怀疑整个采集,比少一个数字糟得多。

判据是 `DeviceInterface.is_logical`:**`if_type` 优先,空了退回名字前缀**。
两套都要 —— `if_type` 是 SNMP 采来的、最可靠,但**它可能是空的**(SSH 通道、
或者某些固件不报;实测这个库里一半的口是空的)。空的时候当成实体口会把
Vlan 接口混进"能数出来的口"里。

⚠ 这条判定又是**三份实现**(模型属性 / `filters.py` 的 SQL / summary 里的
Python 计数)。summary 那份直接用属性(接口总数几十到几百,在 Python 里数
完全够);SQL 那份是给筛选用的,**改属性时要一起改** —— 否则页面标 104 个、
筛选筛出 72 个,不报任何错。

概览上几个数要**加得起来**:`total = 通 + 人为关闭 + 该通没通 + 未采到`,
而 `total = 实体口 + 逻辑口`。原来只有 `total` 和 `up`,人一减发现少了 20 个
口不知道去哪了 —— 那 20 个是人为关掉的,而那个数在页面上根本不存在。
...
- `devices/sdwan.py`:**一台真 FortiGate 都没接过**。已验证的是三种响应
  形状的解析、SSH 输出的解析、dead 时丢弃延迟值、三态处理、以及
  "开在非 Fortinet 上要被两处校验挡住"。真机上最可能要改的是
  `fortigate_api.fetch_sdwan_health()` 里那两个端点名和字段别名
- `idrac/redfish.py` 的 `ENDPOINTS`(iDRAC 7/8 上 `$expand` 不支持、
  ...
  Dell 私有的,固件大版本间会挪位置)。**一台真 iDRAC 都没接过** ——
  已验证的是解析器(拿构造的真实格式 JSON 逐项跑过)、三态处理、
  阈值判定的全部分支、同 kind 合并、以及 migration 在真 PostgreSQL 上的
  正反向执行和那条部分唯一索引对 idrac 来源确实生效
