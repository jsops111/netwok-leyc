...
配置中心 → 服务器 → 新建。**只要一个能登录的 SSH 账号,不用在机器上装东西。**
不需要 root —— 采集只读 `/proc`、跑 `df` 和 `ps`。

| 填什么 | 注意 |
|---|---|
| 地址 / 端口 / 用户名 | 端口默认 22 |
| 密码 **或** 私钥 | 无人值守场景用私钥。私钥带口令的话「私钥口令」必须填,不填的报错会指向"密钥格式不对",很难查 |
| 采集频率 | 默认 60 秒。每次是一次完整的 SSH 握手,最小 15 秒 |
| 流量统计网卡 | **留空**就好 —— 自动取默认路由那块。虚拟机宿主机上那通常是 `br0`,那是对的 |
| 负载阈值 | 按**每核**填(默认 1.5 / 3.0)。绝对值没有可比性 |

建完点**「测试」**。它会把主机名、发行版、核数、内存、最满的挂载点、
以及**流量统计走哪块网卡**一起打出来 —— 网卡挑错了(比如挑到 `docker0`)
在那一行就能看出来。

⚠ **worker 容器要能连到目标机的 22 端口。**容器走 bridge 网络,
目标机的防火墙看到的源地址是 **Docker 宿主机的 IP**,不是容器 IP ——
开放策略时按宿主机 IP 开。

⚠ 刚加进来的服务器**第一拍没有 CPU 使用率**(要两拍 `/proc/stat` 相减),
页面上会写明原因。第二拍开始就有了,这不是故障。

### 4. 开配置备份(可选)

配置中心 → 网络设备 → 编辑 → 「启用配置备份」。

- 需要 **SSH 用户名 + 密码/私钥**(FortiGate 也可以只填 API Token)。
  **和上面的采集通道无关** —— SNMP 拿不到配置文本,所以一台用 SNMP
  采指标的交换机想备份配置仍然要填 SSH 凭据
- Cisco 的 `show running-config` **需要 enable 权限**,所以「enable 密码」要填
- 开关打不开的话看提示:通用 SNMP 画像没有定义备份命令,那款型号不支持备份
- 建完点**「测备份」**。它取一份配置回来但**不存版本**,报错是指向性的
  (需要 enable / 权限不足 / 型号不支持)。成功后它会告诉你行数和首行 ——
  **行数明显偏少说明输出被截断了**

然后去「配置备份」页面点「立即备份」跑第一次,不用等 24 小时。

### 5. 开防火墙策略同步(可选)

配置中心 → 网络设备 → 编辑一台**类型为防火墙**的 FortiGate →
「同步防火墙策略」。

**强烈建议配 API Token。**只有 REST API 拿得到命中计数,而
「这条规则从来没命中过」是策略页面最有价值的结论 —— SSH 通道只能看到配置。
API 还需要 REST API 管理员的 profile 里有 **monitor 读权限**,
只给 cmdb 权限拿不到命中数。

开完去「防火墙策略」页面点「立即同步」,然后看那条
**"这批数据没有命中统计"**的提示在不在。在的话说明走的是 SSH,
或者 API 的 monitor 权限不够。
...
```bash
# 先用配置中心的「测试」按钮,它的报错是指向性的:
#   "SSH 认证失败"                  → 用户名/密码/私钥错
#   "私钥带口令,但「私钥口令」是空的" → 就是字面意思
#   "SSH 连接超时"                  → 网络不通,或者防火墙没放行
#                                     (源地址是 Docker 宿主机 IP,不是容器 IP)
#   "地址解析失败"                  → 填的是域名而容器里解析不了
#   "命令没有产生可解析的输出"        → 登录 shell 不是 sh/bash(csh/fish),
#                                     或者账号被 ForceCommand 限制了
```

**CPU 一直是 `—`**:第一拍必然如此(要两拍 `/proc/stat` 相减)。
第二拍还是 `—` 才是问题 —— 看服务器面板上那行提示。

**流量一直是平的 / 是 0**:多半是主网卡挑错了。展开那台服务器的明细看
「网卡」那一块,标着「主」的那块是不是真正在跑流量的那块。
不对就在配置里显式填「流量统计网卡」。

**某个挂载点没出现在列表里**:虚拟文件系统(tmpfs / overlay / squashfs)和
容器路径(`/var/lib/docker/*`、`/var/lib/kubelet/*`)是有意跳过的,
它们不是"真磁盘"。

### 配置备份失败 / 备份出来的东西不对

```bash
docker compose --env-file .env.docker logs worker | grep -i backup
```

| 症状 | 原因 |
|---|---|
| 「型号 xx 的画像没有定义备份命令」 | 那款型号不支持备份,换在册型号或去 `profiles.py` 补 `backup_cli` |
| 「设备拒绝了备份命令」 | Cisco 的 running-config 需要 enable,「enable 密码」要填 |
| 「`show running-config` 没有输出」 | 同上,或者账号权限不足 |
| 「API 拒绝访问(403)」 | FortiGate 备份需要 profile 里给 **系统 → 配置** 的读权限,只给 monitor 不够 |
| **版本列表一天多一条,但每条都"没改什么"** | 清洗规则没盖住这款固件的易变行 —— 看两个版本的 diff,把那些行的正则加到画像的 `backup_volatile` 里 |
| **下载下来的配置少了一半** | 输出被截断了。`devices/backup.py` 里的 `hard_limit`(180 秒)对超大配置可能还不够 |

**「立即备份」点了没反应**:它是排进队列异步跑的(一份大配置要一两分钟),
不是同步执行 —— 同步做的话 HTTP 请求会先超时。刷新页面看版本列表。

### 防火墙策略同步不上 / 命中数是空的

「命中」那一列显示**「未知」**说明这批数据没有命中计数:

- 走的是 SSH 通道(SSH 拿不到命中数)—— 配上 API Token
- 或者 API Token 有了但 profile 缺 **monitor 读权限**

**「未知」和「从未命中」是两个结论**,页面刻意分开显示:
前者不能拿去删规则。所以那个「只看从未命中的」开关在没有命中统计时是禁用的。

同步失败时**不会清空已有快照** —— 页面上还能看上一次的规则,
只是标明"数据截止于什么时候"。防火墙连不上恰恰是最需要查规则的时候。

### 磁盘涨得快

原始秒级样本是主要占用。一条 1 秒频率的线路一天约 86400 行 ≈ 20MB。

**第二位通常是配置备份**(`netcheck_devicebackup`):一份配置几十 KB 到几 MB,
每台留 `backup_keep` 个版本。紧的时候在设备配置里把「保留版本数」调小 ——
它不按天算,按**变更次数**算(配置没变不新增版本),所以调到 10 通常还够回溯很久。
